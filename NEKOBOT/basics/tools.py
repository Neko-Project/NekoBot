...
async def text2piiic_with_link(text: str, fontsize=40, x=20, y=40, spacing=15):
    ...
    for block in blocks:
        ...
        qr_img = IMG.open(
            f"./statics/temp/tempQrcodeWithLink{block_count}.jpg")
...
async def get_final_text_lines(text: str, text_width: int, font: ImageFont.FreeTypeFont) -> int:
    ...
    for line in lines:
        if not line:
            line_count += 1
            continue
        line_count += int(math.ceil(float(font.getsize(line)
                                          [0]) / float(text_width)))
            ...
            temp_img_list.append(
                temp_img := temp_img.resize(
                    (
                        ...
                        int(float(img_height * (final_width -
                                                2 * spacing)) / float(img_width))
                    )
        ...
        if isinstance(element, Image_LocalFile):
            ...
            for char in element.text:
                ...
                if present_x + font.getsize(char)[0] > text_width:
                    present_y += (font_size + spacing)
                    present_x = padding_x
                draw.text((present_x, present_y), char,
...
def make_card(id, card1, card2, card3, choice, new):
    im1 = Image.open("public/card/%s/%s.jpg" % (id, card1))
    im2 = Image.open("public/card/%s/%s.jpg" % (id, card2))
    im3 = Image.open("public/card/%s/%s.jpg" % (id, card3))
    im4 = Image.new("RGB", (im1.width * 3, im1.height))
    if choice == 0:
        im4.paste(im1, (0, 0, 396, 556))
    else:
        im4.paste(im1.convert("L"), (0, 0, 396, 556))

    if choice == 1:
        im4.paste(im2, (396, 0, 396 * 2, 556))
    else:
        im4.paste(im2.convert("L"), (396, 0, 396 * 2, 556))

    if choice == 2:
        im4.paste(im3, (396 * 2, 0, 396 * 3, 556))
    else:
        im4.paste(im3.convert("L"), (396 * 2, 0, 396 * 3, 556))
    im4.save("temp/card.jpg", "jpeg")
...
if __name__ == "__main__":
    ...
    start = time.time()
    loop.run_until_complete(messagechain_to_img(
        message=message, max_width=1400))
